...
class TestOryxSchemaSCD2_B(TestCase):
    ...
    def test_oryxloss_schema_rolling(self):
        ...
        input_files = [
                       "2025-04-17_attack-on-europe-documenting-equipment_parsed_cleaned.csv",
                       "2025-04-18_attack-on-europe-documenting-equipment_parsed_cleaned.csv",
                       "2025-04-19_attack-on-europe-documenting-equipment_parsed_cleaned.csv",
                       "2025-04-20_attack-on-europe-documenting-equipment_parsed_cleaned.csv",
                       "2025-04-17_attack-on-europe-documenting-ukrainian_parsed_cleaned.csv",
                       "2025-04-18_attack-on-europe-documenting-ukrainian_parsed_cleaned.csv",
                       "2025-04-19_attack-on-europe-documenting-ukrainian_parsed_cleaned.csv",
                       "2025-04-20_attack-on-europe-documenting-ukrainian_parsed_cleaned.csv"
                       ]
        # input_files = ["2025-04-25_parsing_test_1__-attack-on-europe-documenting-ukrainian-__parsed.csv"]
        ...
        tempdir = Path("D:\\ProgBase\\longrow_to_db\\tests\\data")
        test_db = "wartracker_regression.db"
        test_db = f"{prefix}_wartracker_regression_htest_ukr_17-18-19_alltempsconst.db"
        ...
        query_loss_item_2 = """SELECT *
                                     FROM loss_item li INNER JOIN proofs p ON li.proof_id = p.id
                                     INNER JOIN category_names cn ON li.category_id = cn.id"""
        query_loss_item_3 = """SELECT * FROM loss_item"""
        query_proofs = """SELECT proof FROM proofs ORDER BY proof"""
        query_categories = """SELECT * FROM category_names ORDER BY category"""
        ...
        loss_df = pd.read_sql_query(query_loss_item, conn)
        loss_df_2 = pd.read_sql_query(query_loss_item_2, conn)
        loss_df_3 = pd.read_sql_query(query_loss_item_3, conn)
        proof_df = pd.read_sql_query(query_proofs, conn)
        category_df = pd.read_sql_query(query_categories, conn)
        loss_df["proof"] = loss_df["proof"].str.strip()  # it seems sometimes trailing spaces are added by pandas query?

        master_df.to_csv(prefix + "test_master.csv")
        summary_df.to_csv(prefix + "test_summary.csv")
        loss_df.to_csv(prefix + "test_loss.csv")
        loss_df_2.to_csv(prefix + "test_loss_2.csv")
        loss_df_3.to_csv(prefix + "test_loss_3.csv")
        proof_df.to_csv(prefix + "test_proof.csv")
        category_df.to_csv(prefix + "test_category.csv")
